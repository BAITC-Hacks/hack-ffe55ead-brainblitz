...
Целевой контракт `2.0`, данные `city-v1`, правила `five-directions-v1`. Согласован со [спецификацией](spec.md). Статус: договор для следующего этапа реализации; не описание уже запущенного сервера.

## 1. Совместимость и общие соглашения

Контракт согласован между [спецификацией](spec.md), [архитектурой](architecture.md) и README до реализации. Публичные пути: `/api/config`, `/api/simulate`, `/api/analyze`, `/health`. Старые пути без `/api` не поддерживаются как aliases. В текущем этапе приложение и модели Pydantic не создаются.

Оба POST принимают `SimulationRequest` — объект с единственным полем `selections`, массивом решений. Слово «список решений» означает этот массив внутри объекта, а не bare JSON array. Поле `decisions` не используется. Такой формат сохраняет уже выбранное имя и не добавляет клиентских параметров расчёта.

Имена целевых моделей: `SimulationResult` и `AnalysisResult`. Это названия структур Python/Pydantic, не дополнительные обёртки в JSON. Все значения берутся из серверного датасета; клиент не может задавать стоимость, эффект или Score.

Контракт `2.0` заменяет `1.0` из предыдущего этапа из-за смены путей. Версии данных `city-v1` и правил `five-directions-v1` сохраняются, числовые примеры не меняются. Черновой backend другой ветки не является источником правил данного контракта.
...
| GET | `/api/config` | 200 | Полный фиксированный каталог и исходные данные |
| POST | `/api/simulate` | 200 | Валидация и независимый от AI расчёт |
| POST | `/api/analyze` | 200 | Повторный расчёт тех же решений и AI-разбор |
...
Изменяется только район M7. Этот запрос также используется в примере `/api/analyze`.
...
Тело — SimulationRequest. Валидация и вычисление не вызывают AI. Успешный ответ SimulationResult, HTTP 200. Ниже полный результат для B:
...
Тело — тот же SimulationRequest. Сервер заново валидирует и вычисляет SimulationResult; браузер не передаёт ему готовый результат. Только после успешной валидации выполняется один AI-запрос с рассчитанными фактами.

AnalysisResult содержит обязательные поля: `contract_version`, `dataset_version`, `rules_version`, нормализованные `selections`, `baseline_score`, `final_score`, `status`, `analysis`, `unavailable_reason`, `message`. Дополнительных полей нет. Версии и числовые значения совпадают с результатом `/api/simulate` для того же набора.
...
| `dataset_version` | string | `city-v1` |
| `rules_version` | string | `five-directions-v1` |
| `selections` | Selection[5] | Нормализованные решения; городская мера с district_id=null |
| `baseline_score` | number | Получен повторным серверным расчётом |
| `final_score` | number | Получен повторным серверным расчётом |
| `status` | enum | `ok` или `unavailable` |
| `analysis` | Analysis или null | Объект только при status=ok |
| `unavailable_reason` | enum или null | null при ok, код причины при unavailable |
| `message` | string | Пустая при ok; безопасное сообщение при unavailable |

Версии, selections, числовые поля и статус формирует сервер. От LLM ожидается только объект Analysis. Ответ модели не может перезаписать поля AnalysisResult или SimulationResult.

status — `ok` или `unavailable`. При ok: analysis — Analysis, unavailable_reason — null, message — пустая строка. При unavailable: analysis — null, unavailable_reason — одно из значений таблицы ниже, message — безопасное русское сообщение. Внешние технические детали и ключи не выводятся.
...
`evidence_paths` — JSON Pointer относительно корня **SimulationResult**, заново рассчитанного сервером для этого запроса, без префикса `/simulation`. Пример `/districts/4/after/S1` означает S1 Нуры после мер. Путь должен существовать; ссылки на сам AI-текст не допускаются. Они проверяются сервером; корректная ссылка ещё не гарантирует верность её интерпретации. UI использует тот же результат `/api/simulate` только при совпадении попытки, selections и версий.
...
Недоступность AI не превращает допустимый набор в ошибку расчёта. Сервер не выдумывает успешный разбор. Уже показанный `/api/simulate` остаётся на экране.
...
Общий timeout AI — целевые 12 секунд, без автоматических повторов. Режим unavailable — деградация, а не выполнение обязательной AI-функции: приёмка требует хотя бы успешных живых разборов A/B. Ошибки валидации для `/api/analyze` совпадают с `/api/simulate` и не вызывают провайдера.
...
1. Загрузить `/api/config`, построить форму из каталога; не хранить альтернативные цены во frontend.
...
4. Вызвать `/api/simulate`, сразу показать числа. Невалидный набор не отправлять в `/api/analyze`.
5. Вызвать `/api/analyze` с тем же snapshot. AI-loading относится только к текстовому разбору.
...
## 9. Версии и фиксированные ID районов

| Версия | Текущее значение | Когда меняется |
| --- | --- | --- |
| `contract_version` | `2.0` | Меняются публичные пути, имена, типы или семантика полей API |
| `dataset_version` | `city-v1` | Меняются исходные показатели, население, веса, каталог, стоимость или эффекты |
| `rules_version` | `five-directions-v1` | Меняются формула, порог, горизонт, синергии, несовместимости или правила валидации |

Если правка затрагивает несколько областей, обновляются соответствующие версии одновременно. Версии возвращаются в ConfigResponse, SimulationResult и AnalysisResult. Клиент не передаёт их как способ выбрать другую модель; для MVP активен один набор правил. `/health` сохраняет минимальный ответ status=ok, ошибки 422 используют отдельную структуру detail.

| DistrictID | Отображаемое название | Индекс в массиве districts |
| --- | --- | --- |
| `esil` | Есиль | 0 |
| `almaty` | Алматы | 1 |
| `saryarka` | Сарыарка | 2 |
...
Сервер не принимает русские названия, другие транслитерации или ID с изменённым регистром вместо DistrictID. Названия в интерфейсе не меняют ID.

## 10. Проверка контракта
...
- Все evidence_paths из примера `/api/analyze` разрешаются относительно SimulationResult B.
- Перестановка selections не меняет результат и его канонический порядок; повторные запросы не изменяют базу.
- Пустой набор не является способом запросить базу: baseline берётся из `/api/config`.
- Недопустимый пример документа возвращает CATEGORY_COVERAGE, а не 56.54307.
- Реализация будущего backend должна соответствовать этому контракту; проверки документов не означают, что API-тесты приложения уже проходят.
